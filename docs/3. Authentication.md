...
def _req(self, method: str, path: str, params: dict, to_sign: bool):
    self._logger.debug(f'_req({method}, {path}) with params = {pprint.pformat(params)}')
    url = API_REST_URL + path
    headers = self.construct_header(method, path, params, to_sign)
    with self._session as session:
        try:
            ...
            self._logger.error(f'(method={method}, path={path}, params={params}, headers={headers}) '
                                f'error: {e} | timeout={self._timeout}')
            return None
        if rsp:
...
def construct_header(self, method: str, path: str, params: dict, to_sign: bool) -> Dict[str, str]:
    ...
    header: dict = {'EXCHANGE-API-KEY': self._api_key,
                    'EXCHANGE-API-TIMESTAMP': ts}
    if to_sign:
        header.update({'EXCHANGE-API-SIGN': self._signature(method, path, params, ts)})
    self._logger.debug(f'header = {header}')
    return header
	
def signature(priv_key: Ed25519PrivateKey, method: str, path: str, param: dict, ts: str) -> str:
    """
    DFXL signature function
    :return: signature
    """
    logger = logging.getLogger('lib.dfx_util.signature')
    ...
    logger.debug(f'msg to be signed: {z}')
    res = base64.b64encode(priv_key.sign(z)).decode()
    logger.debug(f'signature: {res}')
    return str(res)

def _handle_response(rsp: requests.Response):
    logger = logging.getLogger('_handle_response')
    if rsp.status_code == 200:
        ...
        logger.error(f'failed with {rsp.text}')
        return None
