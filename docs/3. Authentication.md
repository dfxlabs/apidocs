...
PRIVATE_KEY_PATH = 'YOUR_PRVIATE_KEY_PATH'
...
message = dict()
message["param"] = "&".join(f"{key}={value}" for key, value in params.items())
message["path"] = path
message["method"] = method
message["timestamp"] = str(int(time.time() * 1e3))

signing_str = "&".join(f"{key}={value}" for key, value in sorted(message.items()))
signature = private_key.sign(signing_str.encode('utf-8'))
signature_base64 = base64.b64encode(signature).decode()

headers = {
    'EXCHANGE-API-KEY': API_KEY,
    'EXCHANGE-API-SIGN': signature_base64,
    'EXCHANGE-API-TIMESTAMP': timestamp
}

response = requests.post(
    f'https://sim-api.dfx.hk{path}',
    headers=headers,
    params=params
)
...
if response.status_code == 200:
    ...
    print("Fail:", response.status_code, response.json())
```
